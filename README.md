...
v0.8.109 (2026-07-09)
- `filters.yaml` 제거. 개별 툴 `환경 설정` 내에서 설정하는 것으로 변경
- 프론트엔드 설정에 서버 설정(pmh_config.yaml) 수정 기능 추가
- 서버 로그 방식을 print에서 python logging 적용으로 변경
- 서버 로그(툴)에 tool id 대신 tool name 표시로 변경
- 서버(노드) 설정 항목 추가(샘플 YAML 확인)
  - `ENV_TYPE`: `ff`(디폴트. 로그에서 timestamp 생략) / `standalone`(로그에 timestamp 표시). 실행 환경에 맞게 설정
  - `LOG_LEVEL`: `INFO`(디폴트) / `DEBUG`. API/Relay 로그는 DEBUG 레벨로 변경
  - `TZ`: 타임존. 디폴트는 `Asia/Seoul`. 로그 timestamp에 적용
  - `PUID`/`GUID`: 디폴트는 `-1`(변경 없음). PMH가 생성/저장하는 파일의 PUID/GUID 변경
- 툴 리스트 로딩시 작업 상태(캐시) 업데이트 로직 수정
- 프론트엔드 일반/SHIFT 클릭 변경 메뉴 JS 툴팁 구현
