...
v0.9.120 (2026-09-07)
- 백엔드 설정에서 섹션 지정시 범위 지정 가능: ex. `1-5, 7, 9`
- 목록에서 다중 항목 선택시 툴바에 헬퍼 액션 메뉴 추가
- 목록 뷰포트 로직 개선
- 툴/작업 상태 추적 등의 로직 개선
  - 백엔드: 부팅시 로딩 후 인메모리 캐시 관리
  - 프론트엔드: 기존 주기적 폴링 -> SSE 도입(실시간 반영/http 리퀘스트 감소)
  - 백엔드/프론트엔드 간의 상태 체크 로직 개선
- 일부 중복/불필요한 토스트 알림 제거
- 이미지 편집 모달 수정: 비율 템플릿, URL 직접 입력 기능 추가
- 기타 개선, 버그 수정
