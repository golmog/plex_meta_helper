...
v0.8.88 (2026-05-08)
- 3자리 시즌 `YAML/TMDB 적용` 로직 개선
- 기본 파싱 규칙 보완

v0.8.87 (2026-05-03)
- plex 서버 과부하/무응답(50x 에러) 시 재시도 방어 로직 추가
- 매칭 큐 로직(백엔드), UI 반영(프론트엔드) 로직 개선
- 스마트 스캐너: v0.8.29
  - 쇼 3자리 시즌 YAML 적용전 메타 새로고침 실행
  - 메타 갱신 후 3자리 시즌 YAML 자동 적용

v0.8.86 (2026-04-28)
- Plex url 접근시 에이전트 지정

v0.8.85 (2026-04-26)
- UI 미디어 정보 패널 유실 방어 코드 추가
- 쇼/시즌 합산 재생시간 표시

v0.8.84 (2026-04-25)
- `YAML/TMDB 반영: 백엔드 실행으로 변경, vfs/refresh 우선 실행
...
v0.8.82 (2026-04-22)
- UI 업데이트 로직 개선, 버그 수정
- 메타 갱신으로 인한 병합시 후행 처리 로직 개선(404 방지)
- 툴(배치/스마트 스캐너)에서 메타 새로고침 조건 변경(축소)

v0.8.81 (2026-04-21)
- 매칭시 제목/연도 검증 스킵 옵션 추가: 프론트엔드 설정, 툴 작업 실행 옵션
- 매칭 로직 개선, 버그 수정
- 서버 업데이트 로직 버그 수정
- 기타 개선, 버그 수정
