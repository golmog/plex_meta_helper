...
GLOBAL_DASHBOARD_CACHE = {"running": [], "cron": [], "last_updated": 0}
...
def core_natural_sort(data_list, default_sort):
    if not data_list or not default_sort:
        return data_list

    def n_key(s):
        return [
            text.zfill(10) if text.isdigit() else text.lower()
...
def create_discord_notifier(base_dir, tool_id, server_id, global_config):
    ...
    def send_discord_notify(title, message="", color_hex="#51a351", tool_vars=None):
        ...
        if not current_opts.get("discord_enable", True):
            return

        url = current_opts.get("discord_webhook", "").strip() or (
            global_config.get("discord_webhook", "").strip() if global_config else ""
        )
        if not url:
            return
        ...
        raw_footer = current_opts.get(
            "discord_template_footer", "Plex Meta Helper - {tool_id} | {server_name}"
        )
        ...
        color_int = (
            int(color_hex.lstrip("#"), 16) if color_hex.startswith("#") else 5349201
        )

        embed = {"title": final_title, "description": final_body, "color": color_int}
        ...
        extra_data = {}
        if final_bot_name:
            extra_data["username"] = final_bot_name
        if avatar_url:
...
def match_cron(cron_expr, dt):
    ...
    return (
        match_part(parts[0], dt.minute)
        and match_part(parts[1], dt.hour)
        and match_part(parts[2], dt.day)
        and match_part(parts[3], dt.month)
        and match_part(parts[4], dt.isoweekday() % 7)
    )
...
def start_scheduler_daemon(global_config):
    ...
    try:
        ...
        if os.path.exists(task_logs_dir):
                ...
                if f_name.endswith("_task.db"):
                    ...
                    try:
                        with sqlite3.connect(db_file, timeout=2.0) as conn:
                            c = conn.cursor()
                            c.execute(
                                "SELECT count(*) FROM task_info WHERE state IN ('running', 'pending')"
                            )
                            if c.fetchone()[0] > 0:
                                c.execute(
                                    "UPDATE task_info SET state='error' WHERE state IN ('running', 'pending')"
                                )

                                stamp = datetime.now().strftime("%H:%M:%S")
                                c.execute(
                                    "INSERT INTO logs (log_text) VALUES (?)",
                                    (
                                        f"[{stamp}] [System] 서버 강제 종료(재시작)가 감지되어 이전 작업을 중단 상태(Error)로 변경했습니다.",
                                    ),
                                )
            ...
            if ghost_count > 0:
                print(
                    f"[PMH Cleanup] 총 {ghost_count}개의 툴에서 중단된 유령 작업(Ghost Tasks)을 정리했습니다."
                )
...
def _update_dashboard_cache(tools_dir, task_logs_dir, base_dir):
    ...
    if not os.path.exists(tools_dir) or not os.path.exists(task_logs_dir):
        return
...
def _execute_scheduled_tasks(global_config, now):
    ...
    for file in os.listdir(task_logs_dir):
        if file.endswith("_options.db"):
            ...
            if not opts.get("cron_enable", False):
                continue
            ...
            if not match_cron(cron_expr, now):
                continue
            ...
            if task_state and task_state.get("state") == "running":
                print(
                    f"[PMH Scheduler] '{tool_id}' (서버:{server_id[:8]}) 이미 작업이 실행 중이므로 스킵합니다."
                )
                continue

            print(
                f"[PMH Scheduler] '{tool_id}' (서버:{server_id[:8]}) 크론 조건({cron_expr}) 달성. 워커 스레드를 트리거합니다."
            )
            ...
            try:
                ...
                core_api = {
                    ...
                    "cache": data_mgr,
                    "options": opts,
                    "notify": create_discord_notifier(
                        base_dir, tool_id, server_id, global_config
                    ),
                ...
                if (
                    ...
                    if not tool_lock.acquire(blocking=False):
                        print(
                            f"[PMH Scheduler] '{tool_id}' (서버:{server_id[:8]}) 이전 작업이 진행 중이어서 이번 스케줄은 건너뜁니다."
                        )
                        continue
...
def get_db_connection(db_path):
    ...
    try:
        conn = sqlite3.connect(
            f"file:{db_path}?mode=ro", uri=True, timeout=10.0, isolation_level=None
        )
        yield conn
...
def is_season_folder(folder_name):
    ...
    if re.match(r"^(season|시즌|series|s)\s*\d+\b", name_lower):
        return True
    if re.match(r"^(specials?|스페셜|extras?|특집|ova|ost)(\s*\d+)?$", name_lower):
        return True
    ...
    try:
        with get_db_connection(db_path) as conn:
            ...
            try:
                ...
                if check_multi_path:
                    cursor.execute(
                        f"SELECT id, metadata_type FROM metadata_items WHERE id IN ({placeholders})",
                        ids,
                    )
                ...
                for (
                    ...
                    if rk not in result_map:
                        ...
                        if raw_data:
                            ...
                            if (
                                "BT2020" in raw_upper
                                or "SMPTE2084" in raw_upper
                                or "HLG" in raw_upper
                                or "HDR10" in raw_upper
                            ):
                                hdr_badges.add("HDR")
                        ...
                        if hdr_badges:
                            sorted_badges = sorted(
                                list(hdr_badges), key=lambda x: 0 if x == "DV" else 1
                            )
                            video_badge = (
                                video_badge + " " + "/".join(sorted_badges)
                                if video_badge
                                else "/".join(sorted_badges)
                            )
                        if video_badge:
                        ...
                        if sub_data:
                            ...
                            for s in streams:
                                parts = s.split("|")
                                if len(parts) >= 4:
                                    s_id, s_lang, s_codec, s_url = (
                                        parts[0],
                                        parts[1].lower(),
                                        parts[2].lower(),
                                        parts[3],
                                    )
                                    if s_lang.startswith("kor") or s_lang.startswith(
                                        "ko"
                                    ):
                                        has_sub = True
                                        score = 0
                        ...
                        if has_sub:
                            tags.append("SUB")
                        elif filepath and re.search(
                            r"(?i)(kor-?sub|자체자막)", filepath
                        ):
                            tags.append("SUBBED")
...
def handle_media_detail(rating_key, db_path):
    ...
    try:
        with get_db_connection(db_path) as conn:
            ...
            try:
                cursor.execute(
                    "SELECT metadata_type, guid, library_section_id FROM metadata_items WHERE id = ?",
                    (rating_key,),
                )
                    ...
                    for row in cursor.fetchall():
                        if row and row[0]:
                            ...
                            if m_type == 8:
                                ...
                                if re.match(
                                    r"^cd\s*\d+", os.path.basename(album_dir).lower()
                                ):
                            ...
                            dir_key = (
                                os.path.normpath(target_dir).replace("\\", "/").lower()
                            )
                            ...
                            if m_type in (2, 3) and is_season_folder(
                                os.path.basename(target_dir)
                            ):
                                parent_path_original = os.path.dirname(target_dir)
                                parent_key = (
                                    os.path.normpath(parent_path_original)
                                    .replace("\\", "/")
                                    .lower()
                                )
                ...
                if m_type == 9:
                    ...
                    for row in cursor.fetchall():
                        ...
                        if f_path:
                            ...
                            dir_path_original = os.path.dirname(raw_file)
                            dir_key = (
                                os.path.normpath(dir_path_original)
                                .replace("\\", "/")
                                .lower()
                            )
                            if dir_key not in seen_paths:
                        ...
                        tracks.append(
                            {
                                "t_id": t_id,
                                "t_num": track_display_num,
                                "t_title": t_title or "Unknown Track",
                                "a_codec": (a_codec or "").upper(),
                                "a_bitrate": a_bitrate or 0,
                                "file": f_path or "",
                                "part_id": part_id,
                                "has_lyric": has_lyric > 0,
                            }
                        )
                ...
                for row in cursor.fetchall():
                    ...
                    cursor.execute(
                        "SELECT id, language, codec, url FROM media_streams WHERE media_part_id = ? AND stream_type_id = 3",
                        (part_id,),
                    )
                    subs = [
                        {
                            "id": s[0],
                            "languageCode": (s[1] or "und").lower()[:3],
                            "codec": s[2] or "unknown",
                            "key": s[3],
                            "format": s[2] or "unknown",
                        }
                ...
                cursor.execute(
                    "SELECT text, time_offset, end_time_offset FROM taggings WHERE metadata_item_id = ? AND text IN ('intro', 'credits')",
                    (rating_key,),
                )
                markers = {
                    tag_text: {"start": start_offset, "end": end_offset}
...
class CoreTaskManager:
    def __init__(self, base_dir, tool_id, server_id="default"):
        ...
        self.db_file = os.path.join(
            base_dir, "task_logs", f"{tool_id}_{server_id}_task.db"
        )
    ...
    def _setup_db(self):
        with self._get_conn() as conn:
            c = conn.cursor()
            c.execute(
                "CREATE TABLE IF NOT EXISTS task_info (state TEXT, progress INTEGER, total INTEGER, task_data TEXT)"
            )
            c.execute(
                "CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, log_text TEXT)"
            )
            c.execute("SELECT count(*) FROM task_info")
            if c.fetchone()[0] == 0:
                c.execute(
                    "INSERT INTO task_info (state, progress, total, task_data) VALUES ('completed', 0, 0, '{}')"
                )
        ...
        with self._lock:
            ...
            try:
                with self._get_conn() as conn:
                    c = conn.cursor()
                    c.execute(
                        "SELECT state, progress, total, task_data FROM task_info LIMIT 1"
                    )
                    row = c.fetchone()
                    if not row:
                        return None
                    ...
                    c.execute(
                        "SELECT log_text FROM (SELECT id, log_text FROM logs ORDER BY id DESC LIMIT 50) sub ORDER BY id ASC"
                    )
            ...
            try:
                with self._get_conn() as conn:
                    ...
                    task_data_str = json.dumps(
                        data.get("task_data", {}), ensure_ascii=False
                    )
                    c.execute(
                        "UPDATE task_info SET state=?, progress=?, total=?, task_data=?",
                        (
    ...
    def init_task(self, task_data):
        with self._lock:
            ...
            with self._get_conn() as conn:
                ...
                c.execute(
                    "UPDATE task_info SET state='running', progress=0, total=?, task_data=?",
                    (
        ...
        with self._lock:
            ...
            with self._get_conn() as conn:
                ...
                if progress is not None and total is not None:
                    c.execute(
                        "UPDATE task_info SET state=?, progress=?, total=?",
                        (state, progress, total),
                    )
                elif progress is not None:
                    c.execute(
                        "UPDATE task_info SET state=?, progress=?", (state, progress)
                    )
                else:
...
class CoreDataManager:
    def __init__(self, base_dir, tool_id, server_id="default"):
        self.db_file = os.path.join(
            base_dir, "task_logs", f"{tool_id}_{server_id}_cache.db"
        )
        os.makedirs(os.path.dirname(self.db_file), exist_ok=True)
        self._lock = threading.Lock()
    ...
    def save(self, res_data):
        ...
        with self._lock:
            with self._get_conn() as conn:
                ...
                c.execute(
                    "INSERT INTO meta (payload) VALUES (?)",
                ...
                c.execute(
                    f"CREATE TABLE data (pmh_id INTEGER PRIMARY KEY AUTOINCREMENT, {col_defs}, pmh_status TEXT DEFAULT 'pending')"
                )
                c.execute("CREATE INDEX idx_status ON data (pmh_status)")

                if "rating_key" in columns:
                    c.execute('CREATE INDEX idx_rk ON data ("rating_key")')
                if "id" in columns:
                    c.execute('CREATE INDEX idx_id ON data ("id")')
    ...
    def load_page(self, page, limit, sort_key=None, sort_dir="asc"):
        with self._lock:
            ...
            with self._get_conn() as conn:
                ...
                c.execute(
                    "SELECT count(name) FROM sqlite_master WHERE type='table' AND name='meta'"
                )
                if c.fetchone()[0] == 0:
                    return None
    ...
    def mark_as_done(self, key_column, key_value):
        with self._lock:
            ...
            with self._get_conn() as conn:
                ...
                if c.fetchone()[0] == 1:
                    c.execute(
                        f"UPDATE data SET pmh_status = 'done' WHERE \"{key_column}\" = ?",
                        (str(key_value),),
                    )
    ...
    def mark_as_error(self, key_column, key_value):
        with self._lock:
            ...
            with self._get_conn() as conn:
                ...
                if c.fetchone()[0] == 1:
                    c.execute(
                        f"UPDATE data SET pmh_status = 'error' WHERE \"{key_column}\" = ?",
                        (str(key_value),),
                    )
    ...
    def mark_keys_as_done(self, key_column, keys_list):
        with self._lock:
            if not os.path.exists(self.db_file) or not keys_list:
                return
            with self._get_conn() as conn:
                c = conn.cursor()
                ...
                try:
                    c.execute(
                        f'UPDATE data SET pmh_status = "done" WHERE "{key_column}" IN ({placeholders})',
                        keys_list,
                    )
                except Exception as e:
...
class CoreOptionsManager:
    def __init__(self, base_dir, tool_id, server_id="default"):
        self.db_file = os.path.join(
            base_dir, "task_logs", f"{tool_id}_{server_id}_options.db"
        )
        os.makedirs(os.path.dirname(self.db_file), exist_ok=True)
        self._lock = threading.Lock()
        ...
        with self._lock:
            ...
            try:
                with self._get_conn() as conn:
                    c = conn.cursor()
                    c.execute(
                        "SELECT count(name) FROM sqlite_master WHERE type='table' AND name='options'"
                    )
                    if c.fetchone()[0] == 0:
                        return {}
        ...
        with self._lock:
            try:
                with self._get_conn() as conn:
                    ...
                    c.execute(
                        "INSERT INTO options (payload) VALUES (?)",
...
def _core_worker_runner(
    module, task_data, core_api, start_progress, tool_id, server_id="default"
):
...
def create_db_api(db_path, sqlite_bin=None):
    def _run_cli_query(query, params=(), is_select=False):
        if not sqlite_bin or not os.path.exists(sqlite_bin):
            raise FileNotFoundError(
                f"Plex SQLite 바이너리를 찾을 수 없습니다. (경로: {sqlite_bin})\npmh_config.yaml의 PLEX_SQLITE_BIN 경로를 확인하세요."
            )
        ...
        try:
            result = subprocess.run(
                cmd, input=final_script.encode("utf-8"), capture_output=True, check=True
            )
    ...
    def smart_query(query, params=()):
        if not query.strip().upper().startswith("SELECT"):
            raise ValueError(
                "Security Error: 'query' API는 SELECT 문만 허용합니다. 쓰기 작업은 'execute'를 사용하세요."
            )
        ...
        if os.path.exists(wal_path):
            ...
            print(
                "[PMH DB] Plex 서버 종료 감지. 안전을 위해 Plex SQLite 바이너리로 SELECT 쿼리를 우회 실행합니다."
            )
    ...
    return {"query": smart_query, "execute": execute_via_bin}
...
def check_update_readiness(base_dir, force_update=False):
    ...
    if not force_update:
        return (
            False,
            len(active_workers),
            f"현재 진행 중인 작업({len(active_workers)}건)이 있습니다.",
        )

    print(
        f"[PMH Core] Force update requested. Sending cancel signals to {len(active_workers)} workers in parallel..."
    )
            ...
            if os.path.exists(db_file):
                try:
                    with sqlite3.connect(db_file, timeout=5.0) as conn:
                        c = conn.cursor()
                        c.execute(
                            "UPDATE task_info SET state='cancelled' WHERE state IN ('running', 'pending')"
                        )
                        c.execute(
                            "INSERT INTO logs (log_text) VALUES (?)",
                            ("🛑 서버 업데이트 요청으로 인해 작업을 중단합니다.",),
                        )
                        conn.commit()
                    print(
                        f"[PMH Core] Cancel signal sent to DB: {tool_id} on {server_id[:8]}"
                    )
    ...
    while time.time() - start_wait < max_wait_seconds:
        ...
        if not still_running:
            print(
                f"[PMH Core] All workers successfully terminated in {time.time() - start_wait:.1f} seconds."
            )
    ...
    print(
        f"[PMH Core Error] Timeout! Following threads did not terminate: {still_running}"
    )
    print(
        "[PMH Core Error] Update aborted to prevent database corruption or incomplete state."
    )

    return (
        False,
        len(still_running),
        f"{max_wait_seconds}초 대기 후에도 {len(still_running)}개의 작업이 종료되지 않았습니다.",
    )
...
def dispatch_request(subpath, method, args, data, global_config):
    ...
    try:
        if subpath == "ping" and method == "GET":
            ...
            if len(parts) >= 3:
                ...
                try:
                    ...
                    if action == "unmatch":
                        ...
                        if target_title:
                            query_params["title"] = target_title
                        if target_year:
                            query_params["year"] = str(target_year)

                        matches_url = f"{plex_url}/library/metadata/{rating_key}/matches?{urlencode(query_params)}"
                        req = Request(
                            matches_url, headers={"Accept": "application/json"}
                        )
                        ...
                        results = matches_data.get("MediaContainer", {}).get(
                            "SearchResult", []
                        )
                        if not results:
                            return {
                                "error": f"No matches found for agent '{target_agent}'"
                            }, 404
                        ...
                        if best_score < 95:
                            return {
                                "error": f"Best match score is too low ({best_score} < 95). Manual match required."
                            }, 404
                        ...
                        if best_match.get("year"):
                            match_params["year"] = str(best_match.get("year"))
            ...
            if not yaml_url:
                return {"error": "info.yaml URL이 제공되지 않았습니다."}, 400
            ...
            if not original_id:
                return {"error": "잘못된 info.yaml 구조입니다. ('id' 필드 누락)"}, 400

            if target_id:
                safe_tool_id = target_id
            else:
                safe_tool_id = (
                    f"{prefix}_{original_id}"
                    if (prefix and not original_id.startswith(prefix + "_"))
                    else original_id
                )
            ...
            return {
                "status": "success",
                "message": f"'{tool_info.get('name', original_id)}' 설치/업데이트 완료!",
            }, 200
            ...
            if os.path.exists(logs_dir):
                for f_name in os.listdir(logs_dir):
                    if f_name.startswith(f"{tool_id}_") or f_name == f"{tool_id}.json":
                        try:
                            os.remove(os.path.join(logs_dir, f_name))
                        except:
                            pass
            ...
            if data is None:
                data = {}
            server_id = (
                args.get("server_id", data.get("_server_id", "default"))
                if data
                else args.get("server_id", "default")
            )
            ...
            if not current_opts.get("sqlite_bin"):
                cfg_sqlite_bin = (
                    global_config.get("plex_sqlite_bin") if global_config else None
                )
            ...
            final_url = (
                cfg_plex_url if str(cfg_plex_url).strip() else data.get("_plex_url", "")
            )
            final_token = (
                cfg_plex_token
                if str(cfg_plex_token).strip()
                else data.get("_plex_token", "")
            )
            ...
            if action == "ui" and method == "GET":
                ...
                if not hasattr(module, "run"):
                    return {"error": "해당 툴에 실행(run) 함수가 없습니다."}, 500
                ...
                if action_type == "reset":
                    ...
                    if not tool_lock.acquire(blocking=False):
                        return {
                            "status": "error",
                            "message": "이전 작업이 아직 종료되지 않았습니다. 잠시 후 다시 시도해 주세요.",
                        }, 400
                    ...
                    try:
                        ...
                        t = threading.Thread(
                            ...
                            args=(
                                module,
                                saved_task["task_data"],
                                core_api,
                                saved_task.get("progress", 0),
                                tool_id,
                                server_id,
                            ),
                            name=target_thread_name,
                    ...
                    if not cached:
                        ...
                        if not cached:
                            return {"error": "캐시된 데이터가 없습니다."}, 404
                    ...
                    if t_data and "logs" in t_data:
                        cached["logs"] = t_data["logs"]
                    ...
                    if c_enable and str(c_expr).strip():
                        print(
                            f"[PMH Core] ⏰ '{tool_id}' 툴 스케줄 등록 완료: {c_expr}"
                        )
                    else:
                        print(
                            f"[PMH Core] ⚙️ '{tool_id}' 툴 설정 저장 완료 (스케줄 비활성)"
                        )
                    return {"status": "success"}, 200
                    ...
                    if action_type in ["preview", "execute"]:
                        print(
                            f"[PMH Core] 툴 '{tool_id}' 메인 워커 작업 라우팅 (Action: {action_type.upper()})"
                        )
                ...
                if saved_task and saved_task.get("state") == "running":
                    ...
                    task_mgr.log(
                        "[System] 사용자 취소 요청. 진행 중인 항목까지만 처리하고 중단합니다."
                    )
                    return {"status": "success"}, 200
